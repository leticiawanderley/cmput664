...
def replace_with_correction(sentence):
  ...
  for e in errors_match:
    sentence = sentence.replace(e[FULL_MATCH], e[CORRECT] if e[CORRECT] else EMPTY)
...
def string_or_empty(string):
  return string if string else SPACE

def check_format(sentence):
  return '</i>' in sentence or '</c>' in sentence

def build_string(first, second, third):
  return string_or_empty(first) + mark_utterance(string_or_empty(second)) + string_or_empty(third)
...
def set_tags_and_deps(nlp, sentence, data_dict, affix, length):
  tags, deps, error_index = tag_sentence(nlp, sentence)
  ...
  for type in ['tag', 'dep']:
    for i in range(length):
      for j in range(length):
        is_the_same = data_dict['correct_trigram_' + type + '_' + str(i)][-1] == data_dict['incorrect_trigram_' + type + '_' + str(j)][-1]
        data_dict[str(i) + '_' + str(j) + '_' + type].append(is_the_same)
...
def add_sentences(data_dict, sentence, error, error_type_index, full_match_index, incorrect, correct, nlp):
  if check_format(correct) or check_format(incorrect):
    return False
  length = 3
  correct_sentence = sentence_formatting(sentence, error[full_match_index], correct)
  incorrect_sentence = sentence_formatting(sentence, error[full_match_index], incorrect)
    ...
    if sentence_match:
      ...
      for e in errors_match:
        if e[INNER_ERROR_TYPE]:
          number_of_inner += 1
          incorrect = build_string(e[INCORRECT], e[INNER_INCORRECT], e[INNER_INCORRECT_SUFFIX])
          correct = build_string(e[INCORRECT], e[INNER_CORRECT], e[INNER_INCORRECT_SUFFIX])
          is_correct_format = add_sentences(data_dict, sentence, e, INNER_ERROR_TYPE,
                                            FULL_MATCH, incorrect, correct, nlp)
          if is_correct_format:
            add_common_data(data_dict, common_data, sentence, current_exam_score)
          else:
            number_of_wrong += 1
        ...
        is_correct_format = add_sentences(data_dict, sentence, e, ERROR_TYPE,
                                          FULL_MATCH, incorrect, correct, nlp)
        if is_correct_format:
          add_common_data(data_dict, common_data, sentence, current_exam_score)
        else:
...
def main(test=False):
  ...
  data_dict = {'student_id': [], 'language': [], 'overall_score': [],
             ...
             'correct_trigram_tag_0': [], 'correct_trigram_tag_1': [], 'correct_trigram_tag_2': [],
             'correct_trigram_dep_0': [], 'correct_trigram_dep_1': [], 'correct_trigram_dep_2': [],
             'incorrect_error_index': [], 'incorrect_sentence': [],
             'incorrect_trigram_tags': [], 'incorrect_trigram_deps': [],
             'incorrect_trigram_tag_0': [], 'incorrect_trigram_tag_1': [], 'incorrect_trigram_tag_2': [],
             'incorrect_trigram_dep_0': [], 'incorrect_trigram_dep_1': [], 'incorrect_trigram_dep_2': [],
  ...
  if test:
    test = './fce-released-dataset/dataset/0100_2000_12/doc1033.xml'
    ...
    for directory in directories:
      if path.isdir(dataset + directory):
        ...
        for file in files:
          a,b, c = get_errors(combine_filename(dataset, directory, file), data_dict, nlp)
          total_errors += a
          wrong += b
          number_of_inner += c
  df = pd.DataFrame.from_dict(data_dict)
