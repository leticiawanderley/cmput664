...
# Plotting top errors by language
```{r top_error_types_by_language}
top_errors_by_language <- data %>%
  group_by(language) %>%
  count(error_type) %>%
  mutate(freq = n/sum(n))

top_errors_by_language <- top_errors_by_language %>%
  group_by(language) %>%
  top_n(1)

top_errors_by_language_distribution <- top_errors_by_language %>%
  filter(error_type %in% unique(top_errors_by_language$error_type))

ggplot(top_errors_by_language_distribution, aes(x = language, y = n)) + 
    geom_point() +
    geom_point(data=top_errors_by_language, colour="red") +
    facet_wrap(~error_type, nrow=3, ncol=3) +
    ggtitle("Top error types distribution over languages") +
...
errors_by_language[which(errors_by_language$freq>0.035),] %>%
    ggplot(aes(x = error_type, y = freq)) + 
    geom_point() +
...
correct_bigrams_by_language[which(correct_bigrams_by_language$freq>0.35),] %>%
    ggplot(aes(x = correct_bigram, y = freq)) + 
    geom_point() +
    facet_wrap(~language, nrow=4, ncol=4) +
